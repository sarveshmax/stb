...
export default function StuckPage() {
  ...
  async function handleManualAdd() {
    ...
    try {
      ...
      if (res.value.length === 0)
        return push("error", "No stuck tokens found for this mint");
  ...
  async function burnToken(t: StuckToken) {
    ...
    if (isNaN(amountNum) || amountNum < 0)
      return push("error", "Invalid Amount");

    if (amountNum > t.balance)
      push("info", "NOTE: Amount Exceeds Stuck Balance");
    ...
    try {
      ...
      push(
        "success",
        `Burn complete. Tx: ${sig.slice(0, 6)}...${sig.slice(-6)}`,
      );
      setStatus(
      ...
      <div className="min-h-screen flex flex-col bg-[#0f0f10] text-white md:ml-64 font-inter">
        <div className="flex-grow p-4 sm:p-6">
          <div className="max-w-5xl mx-auto">
            ...
            {/* Stuck Token Grid */}
            {tokens.length === 0 ? (
              <div className="text-center text-gray opacity-50">
                No stuck tokens added.
              </div>
            ) : (
            ...
            <div className="bg-[#1c1c1e] rounded-xl p-5 mb-6 mt-6 border border-gray-800 shadow">
              ...
              <div className="mt-4 text-xs text-gray-400 border-t border-gray-800 pt-3 leading-relaxed">
                The transaction preview may not show a burn because the token
                account belongs to the Incinerator PDA, not your wallet.
              </div>
            </div>
          </div>
